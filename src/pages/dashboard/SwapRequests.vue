<template>
    <div class="space-y-5">
        ...
        <div class="space-y-4">
            <div v-for="request in requests" :key="request.id" class="space-y-5 rounded-lg bg-white p-5 shadow-3xl">
                <div class="flex flex-wrap items-center justify-between gap-6">
                    <div class="flex flex-wrap items-center gap-5">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div class="space-y-1.5">
                                <h3 class="text-base/5 font-semibold text-black">{{ request.user.first_name }} {{ request.user.last_name }}</h3>
                                <div>Software Engineer</div>
                            </div>
                        </div>
                        <!-- <div
                            v-for="skill in request.user_skills"
                            :key="skill.id"
                        ...
                        >
                            <div class="relative rounded-lg bg-gray-200 pr-7">
                                <div class="rounded-lg bg-[#FEE2E2] px-2 py-1.5">{{ request.skill.title_en }}</div>
                                <icon-check-round-green class="absolute right-1.5 top-1/2 -translate-y-1/2" />
                            </div>
                        </div>
                    </div>
                    <div v-if="request.status == 'pending'" class="flex flex-wrap items-center gap-4">
                        <div
                            @click="showModal(request.skill, request.swap_id, request.skill.id, (status = 'accepted'))"
                        ...
                        <div
                            @click="showModal(request.skill, request.swap_id, request.skill.id, (status = 'rejected'))"
                    ...
                    <div v-else-if="request.status == 'accepted'" class="flex flex-wrap items-center gap-4">
                        <div
                    ...
                    <div v-else-if="request.status == 'completed'" class="flex flex-wrap items-center gap-4">
                        <div
                    ...
                    <div v-else-if="request.status == 'rejected'" class="flex flex-wrap items-center gap-4">
                        <div
                            class="flex items-center gap-2 rounded-lg bg-white px-3 py-[7px] text-left text-xs shadow-3xl ring-1 ring-inset ring-gray-100"
                        >
...
export default {
    ...
    methods: {
        async swapRequests() {
            try {
                const response = await axios.get(`/skill/recieved-swap-requests`);
                this.requests = response.data.data;
                console.log('All users requests', this.requests);
            } catch (error) {
        ...
        async submitRequest() {
            try {
                if (this.status == 'accepted') {
                    ...
                    console.log('swapSkills data', this.swapId,);
