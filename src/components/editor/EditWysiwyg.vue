<template>
    <div id="toolbar" class="border-[1px] w-full shadow-3xl rounded-lg min-h-[128px] border-[#e2e5ec] flex flex-col">
        <div id="body" :style="{ 'min-height': `${height}px` }">
            <div id="editor" v-html="content" ref="editor" @mouseup="saveSelection" @keyup="saveSelection"
                contenteditable :style="{ 'min-height': `${height}px` }"></div>
        </div>
        <div class="flex justify-between items-center px-2  h-[52px] border-t-[1px] border-[#e2e4e9]">
            ...
            <button type="button" @click="updatePost" class="btn">
                <span> {{ $t('update')}} </span>
            </button>
        </div>
    ...
    methods: {
        async submitPost() {
            ...
            try {
                ...
                const response = await axios.post(`/feeds/${this.feedId}/comment`, formData, {
                    headers: {
                ...
                if (response.data.status_code == 200) {
                    ...
                    // this.$refs.editor.innerHTML = "What's happening?"
        ...
        async updatePost() {
            ...
            try {
                ...
                // formData.append('user_id', this.user.id);
                const response = await axios.post(`/feeds/${this.feedId}/update`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
                ...
                if (response.data.status_code == 200) {
                    this.$toastr.success('Comment submitted successfully');
                    this.$emit('updatedPost', editorContent);
                    // this.$refs.editor.innerHTML = this.c
                } else if (response.data.status_code == 422) {
                    this.$toastr.error(response.data.message);
                } else if (response.data.status_code == 400) {
                    this.$toastr.error(response.data.message);
